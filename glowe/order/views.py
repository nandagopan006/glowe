...
def order_detail(request, order_id):
    ...
    ReturnRequest = apps.get_model("return", "ReturnRequest")
    returns = (
        ReturnRequest.objects.filter(order_item__order=order)
        .prefetch_related("images", "order_item__variant__product__images")
        .order_by("-created_at")
    )
...
def cancel_order(request, order_id):
    """
    Cancel an entire order and process refund if applicable.
    """
...
def cancel_order_item(request, item_id):
    ...
    refunded = process_refund(
        ...
        description=f"Refund for cancelled item in Order #{order.order_number} — {item.variant.product.name}",  # noqa: E501
    )
...
def download_invoice(request, order_id):
    ...
    if (
        not invoice["active_items"]
        and not invoice["cancelled_items"]
        and not invoice["returned_items"]
    ):
    ...
    if font_path:
        try:
            ...
            bold_path = (
                font_path.replace("arial.ttf", "arialbd.ttf")
                if "arial.ttf" in font_path
                else font_path
            )
    ...
    lg = ParagraphStyle(
        "lg", fontName=FONT_BOLD, fontSize=18, leading=22, textColor=INK
    )
    rgt = ParagraphStyle(
        "rgt",
        fontName=FONT,
    ...
    hdr = Table(
        [
            [
                ...
                Paragraph(
                    f"<b>INVOICE</b><br/>"
                    f"<font size='8' color='#8c7f77'>#{order.order_number}</font>",  # noqa: E501
                    ParagraphStyle(
                        "inv",
                        fontName=FONT_BOLD,
                        fontSize=14,
            ...
            [
                Paragraph(
                    f"<font color='#8c7f77'>{order.created_at.strftime('%d %B %Y')}</font>",  # noqa: E501
                    sm,
                ),
    ...
    col_w = [
        ...
        55,
        70,
    ]  # Product | Qty | Price | Status | Amount
    ...
    response["Content-Disposition"] = (
        f'attachment; filename="Glowe_Invoice_{order.order_number}.pdf"'
    )
    return response
...
def admin_order_detail(request, order_id):
    ...
    order = get_object_or_404(
        Order.objects.prefetch_related(
            "items__variant__product", "items__returnrequest_set"
        ),
        id=order_id,
    )
    ...
    if has_any_return:
        ...
        if order.order_status != target_status:
            ...
            if not OrderStatusHistory.objects.filter(
                order=order, status=target_status
            ).exists():
                OrderStatusHistory.objects.create(
                    order=order,
                    status=target_status,
                    description=f"Status updated to {target_status} after return check.",  # noqa: E501
                )
