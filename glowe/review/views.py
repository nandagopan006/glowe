...
def admin_review_list(request):
    ...
    pending = Review.objects.filter(
        status="pending", is_deleted=is_deleted_filter
    ).count()
    approved = Review.objects.filter(
        status="approved", is_deleted=is_deleted_filter
    ).count()
    rejected = Review.objects.filter(
        status="rejected", is_deleted=is_deleted_filter
    ).count()
...
def approve_review(request, review_id):
    ...
    if review.status == "approved":
        if request.headers.get("x-requested-with") == "XMLHttpRequest":
            return JsonResponse(
                {"status": "success", "message": "Review already approved"}
            )
    ...
    if request.headers.get("x-requested-with") == "XMLHttpRequest":
        return JsonResponse(
            {"status": "success", "message": "Review approved successfully"}
        )
...
def reject_review(request, review_id):
    ...
    if review.status == "rejected":
        if request.headers.get("x-requested-with") == "XMLHttpRequest":
            return JsonResponse(
                {"status": "success", "message": "Review already rejected"}
            )
    ...
    if request.headers.get("x-requested-with") == "XMLHttpRequest":
        return JsonResponse(
            {"status": "success", "message": "Review rejected successfully"}
        )
