...
def search_products(request):
    ...
    if query:
        ...
        for product in products:
            ...
            variant = product.variants.filter(is_default=True, is_active=True).first()
            if not variant:
...
def search_suggestions(request):
    """
    Returns JSON search suggestions for autocomplete.
    """
    query = request.GET.get("q", "").strip()
    suggestions = []

    if query and len(query) >= 2:
        products = SearchService.get_suggestions(query, limit=5)
