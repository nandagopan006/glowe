...
def edit_profile(request):
    ...
    if request.method == "POST":
        ...
        if phone_number:
            ...
            if (
                ProfileUser.objects.filter(phone_number=clean_phone)
                .exclude(pk=user.pk)
                .exists()
            ):
        ...
        if email != user.email:
            ...
            if (
                ProfileUser.objects.filter(email=email, is_verified=True)
                .exclude(pk=user.pk)
                .exists()
            ):
            ...
            return redirect(
                "verify_email_change", extra_tags="show_on_overview"
            )
...
def change_password(request):
    ...
    if request.method == "POST":
        ...
        if not re.match(password_pattern, new_password):
            messages.error(
                request,
                "Password must be at least 8 characters long, include one uppercase letter, one number and one special character",  # noqa: E501
            )
            return redirect("change_password")
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            exists = Address.objects.filter(
                user=request.user,
                street_address__iexact=address.street_address,
            ...
            if not Address.objects.filter(user=request.user).exists():
                ...
                address.is_default
            ):  # if usr add new add and set def so per need to chnge
...
def edit_address(request, id):
        ...
        if form.is_valid():
            ...
            exists = (
                Address.objects.filter(
                    user=request.user,
                    street_address__iexact=updated.street_address,
                    city__iexact=updated.city,
                    state__iexact=updated.state,
    ...
    return render(
        request, "edit_address.html", {"form": form, "address": address}
    )
